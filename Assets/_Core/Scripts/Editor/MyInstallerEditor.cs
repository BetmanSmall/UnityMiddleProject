...
[CustomEditor(typeof(MyInstaller))]
public class MyInstallerEditor : Editor
{
    private string[] configPaths;
    private string[] configNames;
    private int selectedConfigIndex = -1;

    private SerializedProperty configProviderTypeProperty;

    private void OnEnable()
    {
        configProviderTypeProperty = serializedObject.FindProperty("configProviderType");
        RefreshConfigList();
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.PropertyField(configProviderTypeProperty);

        if (configProviderTypeProperty.enumValueIndex == (int)ConfigProviderType.ScriptableObject)
        {
            ...
            if (configNames.Length > 0)
            {
                ...
                selectedConfigIndex = EditorGUILayout.Popup("Select Config Asset", selectedConfigIndex, configNames);

                if (selectedConfigIndex != oldSelectedIndex && selectedConfigIndex >= 0)
                {
                    // Сохраняем выбранный конфиг в PlayerPrefs
                    PlayerPrefs.SetString("SelectedConfigPath", configPaths[selectedConfigIndex]);
                    PlayerPrefs.Save();

                    Debug.Log($"Selected config: {configNames[selectedConfigIndex]}");
                }
            }
            else
            {
